...
customerOrderRouter.get("/orders", asyncHandler(async (req, res) => {
...
customerOrderRouter.patch("/orders/:orderId/cancel", asyncHandler(async (req, res) => {
    const dbUser = await getDbUserFromReq(req);
    ...
    if (foundOrder.orderStatus === "delivered" ||
        ...
        throw new AppError(400, "Delivered or returned orders cannot be cancelled");
    }
...
customerOrderRouter.patch("/orders/:orderId/return", asyncHandler(async (req, res) => {
    const dbUser = await getDbUserFromReq(req);
    const orderId = String(req.params.orderId || "").trim();
    requireText(orderId, "Order Id is required");
    const order = await Order.findOne({ _id: orderId, user: dbUser._id });
